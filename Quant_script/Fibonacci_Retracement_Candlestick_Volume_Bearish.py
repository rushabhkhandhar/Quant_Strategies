...
def export_annotated_chart(
	...
	fig, axes = mpf.plot(
		...
		figscale=1.2,
		figratio=(16, 9),
		title=f"{symbol} | Fibonacci + Candlestick + Volume Setup (Bearish)",
		ylabel="Price",
		ylabel_lower="Volume",
		returnfig=True,
	)
	...
	raw_levels = [
		...
		("Entry", float(signal["entry_price"]), "#dc2626", "-"),
		("SL1", float(signal["stop_loss_1"]), "#f97316", ":"),
		("SL2", float(signal["stop_loss_2"]), "#f59e0b", ":"),
		("SL3", float(signal["stop_loss_3"]), "#fbbf24", ":"),
...
def compute_trade_levels_short(
	entry_level: float,
	fib_382: float,
	fib_618: float,
	swing_low: float,
	...
	"""Build practical target and stop-loss tiers for short setups."""
	range_size = max(swing_high - swing_low, 1e-9)

	stop_loss_1 = max(current_high, fib_618)
	stop_loss_2 = swing_high
	stop_loss_3 = swing_high + 0.25 * range_size

	target_1 = swing_low
	target_2 = swing_low - 0.272 * range_size
	target_3 = swing_low - 0.618 * range_size
...
def bearish_shooting_star(candle: pd.Series) -> bool:
	...
	return (
		...
		and upper_shadow >= 2.0 * max(body, 1e-9)
		and lower_shadow <= (0.10 * day_range)
		and ((close_price - low_price) / day_range) <= 0.35
	)


def bearish_engulfing(prev_candle: pd.Series, curr_candle: pd.Series) -> bool:
	...
	return (
		...
		and (curr_open - curr_close) > (prev_close - prev_open)
	)
...
def fibonacci_bounce_tomorrow_signal(
	...
	"""Fibonacci retracement bounce setup for next day (bearish).

	Rules:
	1) Build Fib from recent swing high to swing low (downtrend).
	2) Current candle is near 50% or 61.8% retracement zone.
	3) Bearish candlestick confirmation (Shooting Star or Bearish Engulfing).
	...
	prefix = window.loc[:low_idx]
	if len(prefix) < 2:
		return None

	high_idx = prefix["High"].idxmax()
	swing_high = float(prefix.loc[high_idx, "High"])
	swing_low = float(window.loc[low_idx, "Low"])

	if swing_high <= swing_low:
		return None

	swing_pct = ((swing_high - swing_low) / swing_high) * 100.0 if swing_high > 0 else 0.0
	...
	elif bearish_shooting_star(curr):
	...
	trade_levels = compute_trade_levels_short(
...
def build_output_name(as_of_date: date) -> str:
	return f"fibonacci_retracement_candlestick_volume_bearish_tomorrow_{as_of_date.strftime('%d_%m_%Y')}.csv"
...
def main() -> None:
	...
	print(f"\n=== Fibonacci Retracement + Candlestick + Volume Tomorrow Watchlist (Bearish, {universe_name}) ===")
	if results.empty:
		print("No candidates")
	else:
